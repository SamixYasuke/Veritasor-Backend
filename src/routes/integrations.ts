    ...
    try {
      ...
      const connectedSafe = connected.map((i) => ({
        ...
        metadata: {
          ...
          ...Object.fromEntries(
            Object.entries(i.metadata).filter(
              ([key]) =>
                !key.includes("token") &&
                !key.includes("secret") &&
                !key.includes("key"),
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { provider, redirectUri } = connectIntegrationSchema.parse(
        req.body,
      );
      ...
      const existingConnection = existingIntegrations.find(
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const deleted = await deleteById(req.user!.userId, integrationId);
    ...
    try {
      ...
      const safeIntegration = {
        ...
        metadata: {
          // Only include non-sensitive metadata
          ...Object.fromEntries(
            Object.entries(integration.metadata).filter(
              ([key]) =>
                !key.includes("token") &&
                !key.includes("secret") &&
                !key.includes("key"),
